...
let selectedVietnameseVoice = null; // Thêm biến cho giọng Việt
...
// Điền vào dropdown chọn giọng đọc tiếng Trung
...
//Điền vào dropdown chọn giọng đọc tiếng Việt
function populateVietnameseVoiceList() {
  const voiceSelect = document.getElementById("vietnameseVoiceSelect");
  voiceSelect.innerHTML = '<option value="">Tự động</option>';
...
// Lắng nghe sự kiện change của dropdown chọn giọng đọc tiếng Trung
...
// Lắng nghe sự kiện change của dropdown chọn giọng đọc tiếng Việt
document.getElementById("vietnameseVoiceSelect").addEventListener("change", function() {
  const selectedVoiceName = this.value;
  if (selectedVoiceName) {
    selectedVietnameseVoice = voices.find(voice => voice.name === selectedVoiceName);
  } else {
    selectedVietnameseVoice = null; // Đặt lại về tự động
  }
});
...
function speakNext() {
  ...
  // Lấy giọng đọc tiếng Việt (ưu tiên giọng đã chọn)
  let vietnameseVoice = selectedVietnameseVoice;

  // Nếu chưa chọn, tìm giọng mặc định
  if (!vietnameseVoice) {
  ...
  if (vietnameseVoice) vietnameseUtterance.voice = vietnameseVoice; // Áp dụng giọng Việt
